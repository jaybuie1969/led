...
from led.src.baseclasses import LEDModel as __ParentModel__, SignalAggregator as __ParentAggregator__, SignalGenerator as __ParentGenerator__

def run_model(model:__ParentModel__, source:any, samples_to_run:int, figure:Figure=None, line_frame:Line2D=None, time_series:List[any]=None, frames:np.ndarray=None):
	'''
	...
	source : any
		The set of signal sources that will be fed into this model; it can either be an individual object that is a subclass of led.src.baseclasses.SignalGenerator, a list of such objects, or a signal aggregator object
	samples_to_run : int
	...
	elif (not issubclass(type(model), __ParentModel__)):
		raise Exception(f"model argument must be a subclass of led.src.baseclasses.LEDModel, {type(model).__name__} is not")

	if (source == None):
		raise Exception("source argument must be either a SignalAggregator object, or one ore more single signal sources that are subclasses of led.src.baseclasses.SignalGenerator")
	else:
		# Check to see what type of source(s) is/are being used by this function call
		source_type = type(source)
		if (issubclass(source_type, __ParentAggregator__)):
			# This method call uses a signal aggregator object, set the relevant flag to True
			uses_aggregator = True
		elif (source_type == list):
			# For a list of sources, iterate over each one and make sure that they are subclasses of the base Signal Generator subclass

			for i in source:
				if (i is None):
					raise Exception(f"None element found in source argument")
				elif (not issubclass(type(i), __ParentGenerator__)):
			...
			# This method call is going to sum multiple signals together, set the relevant flag to True
			sums_multiple_signals = True
		elif (not issubclass(source_type, __ParentGenerator__)):
	...
	if (samples_to_run == None):
		...
		if (uses_aggregator):
			# source_type is a signal aggregator, check to see if its "finite_signals" attribute is empty
			if (source.finite_signals == []):
				raise Exception(f"with a samples_to_run argument of None, at least one signal in the source argument must have an end_of_signal attribute")
		elif (not sums_multiple_signals):
			# source_type is a signal generator, check to see if it has an "end_of_signal" attribute
			if (not hasattr(source, "end_of_signal")):
				raise Exception(f"source argument of type {type(source).__name__} is invalid with a samples_to_run argument of None, it does not have an end_of_signal attribute")
		else:
			# source_type is a list of signal generators, check to see if at least one has an "end_of_signal" attribute
			if (not any([hasattr(single_source, "end_of_signal") for single_source in source])):
				raise Exception("with a samples_to_run argument of None, at least one signal in the source argument must have an end_of_signal attribute")
	else:
	...
	if (samples_to_run == None):
		while (not (source.end_of_all_finite_signals if (uses_aggregator) else source.end_of_signal if (not sums_multiple_signals) else all([single_source.end_of_signal for single_source in source if (hasattr(single_source, "end_of_signal"))]))):
			if (uses_aggregator):
				# This method call uses a signal aggregator, it is handled differently than a stand-alone signal generator or list of generators

				# It is assumed that the LED model is one that is ready to handle multiple signal outputs from an aggregator
				model.inputs(source.next)
			else:
				# This method does not use an aggregator, add the values together from every source
				value = 0
				if (not sums_multiple_signals):
					# Source is a single signal generator object
					value = source.next
				elif (len(source) > 0):
					# Source is a list of signal generator objects
					value = sum([i.next for i in source])

				# Add the computed value to model
				model.input(value)

			time_series.append(model.newest)
		...
		for i in range(samples_to_run):
			...
			# No matter what type of source was used, get the first value from the model's frame for time_series
			time_series.append(model.newest)
			frames = np.append(frames, [model.frame], axis=0)

			if (display_graph):
				# A graph is being displayed, update the graph object with the model's new frame
				line_frame.set_ydata(model.frame)
				figure.canvas.draw()
				figure.canvas.flush_events()
...
def save_frames(frames:np.ndarray, filename:str):
	'''
	This function takes a numpy array and saves it out as a JSON-formatted text file
	The numpy array is assumed to contain the individual frames of model data from an LED model

	Parameters
	----------
	frames : np.ndarray
		The set of frames that contain the values from an LED model for a series of time-varying data
	filename : str
		The path/name of the file to which frames will be written
	'''

	# Validate all incoming arguments, raising Exceptions if they are not valid

	# Create a Boolean flag to use for the first conditional because numpy arrays produce an error when compared directly to None
	probably_none = True
	try:
		if (frames != None):
			# If the code made it to this point without throwing an Exception, it is not a numpy array, but it is also definitely not None, set probably_none accordingly
			probably_none = False
	except:
		# If this Exception was encountered, it is assumed that it occurred because frames is a numpy array, and checking to see if it is None generated this
		# Exception, thus, it probably is not None
		probably_none = False

	if (probably_none):
		raise Exception("frames argument must be a numpy array")
	elif (type(frames) != np.ndarray):
		raise Exception(f"frames argument must be a numpy array, not an object of type {type(time_series).__name__}")
	...
	# If the method call has made it to this point, all incoming arguments are at least syntactically valid, now save frames
	with open(filename, "w") as file_out:
		json.dump(frames.tolist(), file_out)
...
def save_frames_image(frames:np.ndarray, filename:str):
	...
	# If the method call has made it to this point, all incoming arguments are at least syntactically valid, now create the image from time_series, save it and return it
